...
#define USING_BUTTONS 2  // comment if not using buttons
...
const int N_POTS = 5;           // total numbers of pots (slide & rotary)
const int POT_ARDUINO_PIN[N_POTS] = { A0, A1, A2, A3, A6 };  // pins of each pot connected straight to the Arduino
...
byte cc = 27;      // Lowest MIDI CC to be used
    ...
    if ((millis() - lastDebounceTime[i]) > debounceDelay) {
      ...
      if (buttonPState[i] != buttonCState[i]) {
        ...
        if (buttonCState[i] == LOW) {
          ...
          controlChange(midiCh, 29 + i, midiCState[i]);
          MidiUSB.flush();
...
#elif ATMEGA32U4
        //use if using with ATmega32U4 (micro, pro micro, leonardo...)
        controlChange(midiCh, 21 + i, midiCState[i]);  //  (channel, CC number,  CC value)
        MidiUSB.flush();
